...
StructureData <- function(df, full = TRUE, lowuse = NULL, empty = NULL){
  ...
  if(is.null(lowuse)){
    ...
    low
    CleanData <- function(df, full = TRUE, lowuse = NULL, empty = NULL){
      
      if(is.null(lowuse)){
        lowuse <- 1
      } else {
        lowuse <- paste0(1,lowuse)
      }
      
      if(is.null(empty)){
        empty <-  lowuse
      } else {
        lowuse <- paste0(1,empty)
      }
      ...
      if(full){
        df<- df%>%
          mutate(LowUse = rowSums(.[,-lowuse]), Empty = rowSums(.[,-empty])) %>%
