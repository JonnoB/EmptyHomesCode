StructureData2 <- function(df, lowuse = NULL, empty = NULL, full = TRUE, Business = NULL ){
  ...
  if(!is.null(Business)){
    df <- df %>% bind_rows(SlakelBuss)
    
    Business2 <- Business %>% group_by(LSOA_CODE) %>%
    summarise(HomesB = n()) 
  }
  ...
  if(full){
    ...
    if(!is.null(Business)){
      df <- df %>% left_join(Business2, by="LSOA_CODE") %>%
        mutate(HomesB = as.numeric(HomesB),
          HomesB = ifelse(is.na(HomesB), 0, HomesB),
          Homes = Homes + HomesB) 
       }
