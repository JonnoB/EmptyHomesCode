...
ken2 <- read.xlsx("Copy of Kensington and ChelseaExemptionsLSOA (002).xlsx")
```
...
```{r}
test2 <- CTtaxstock 
pop2 <- pop %>% filter(grepl("Cornwall 008",X3))

#padstow
pad <- left_join(select(test2, ECODE, ALL_PROPERTIES), select(pop2, `Area Codes`, `All Ages`),
          by=c("ECODE" = "Area Codes")) %>% 
  mutate(diff = ALL_PROPERTIES-`All Ages`, ratio = ALL_PROPERTIES/`All Ages`) %>% filter(complete.cases(.))
...
AG2 <- AG2 %>% left_join(., WardDat, by = "Admin_ward_code") %>% 
  filter(Wardname != "Kensal Green")


 

ggplot()+ geom_polygon(data=AG2, aes(long, lat, group = group, fill = vallow), 
colour = alpha("darkred", 1/2), size = 0.7) + scale_colour_gradient2() 
```
...
test <-read_csv("pp-2016.csv", col_names = FALSE )

#quick and dirty loses half the sales due to missing the postcode
test <- test %>% left_join(., PstCdLSOA.raw, by = c("X4"="Postcode")) %>% filter(!is.na(lsoa11cd))


test2 <- test %>% group_by(Admin_ward_code) %>% summarise( MeanPrice = mean(X2), counts =n() )

WardDat2 <- WardDat %>% left_join(., test2) %>% mutate(vallow = LowUse*MeanPrice) %>%
