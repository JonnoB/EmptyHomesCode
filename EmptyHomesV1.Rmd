...
round(table(ChelPrice$class)/nrow(ChelPrice)*100)

set.seed(16544)
Chelseastrapped <- ClassStrapper(ChelPrice, sum(Chelseainfo$LowUse))

Chelseastrapped %>% group_by(class)%>%
  summarise(Counts = mean(Counts)) %>%
  mutate(ratio = Counts/sum(Counts))

Chelseastrapped %>% ggplot(., aes(x= class, y = Counts, fill = class)) + geom_boxplot()
...
set.seed(16540)
ChelseaLSOAstrap1 <- unique(ChelPrice$Admin_ward_code)%>% 
  map(~ ChelPrice %>% 
        filter(Admin_ward_code == .x) %>% 
  ClassStrapper(., sum((filter(ChelseainfoWard, Admin_ward_code == .x))$WardLowUse)) %>%
...
ChelseaLSOAstrap <- ChelseaLSOAstrap1 %>%  
...
ChelseaWardHomesStrap1 <- unique(ChelPrice$Admin_ward_code)%>% 
  map(~ ChelPrice %>% 
        filter(Admin_ward_code == .x) %>% 
  ClassStrapper(., sum((filter(ChelseainfoWard, Admin_ward_code == .x))$WardHomes)) %>%
    mutate(class = as.character(class),
           Counts = as.integer(Counts),
           ID = as.integer(ID)) %>%
      setNames(c("class", paste0("Counts", .x) ,"ID"))
...
test1 <- Chelseastrapped %>% left_join(., ChelseaLSOAstrap, by = c("class", "ID")) %>%
  mutate(ratio = Totals/Counts) 

test1 <- ChelseaWardHomesStrap1  %>% left_join(., ChelseaLSOAstrap, by = c("class", "ID")) %>%
...
test1 %>% ggplot(., aes(x= class, y = RatioExvsAct, fill = class)) + geom_boxplot() + 
  labs(title = "Boxplot of bootstrapped Ratios")


test1 %>% group_by(class) %>%
  summarise(meanC = mean(Counts), 
            meanT = mean(Totals), 
            meanR = mean(ratio))

ChelseaWardHomesStrap1 %>% group_by(class) %>%
  summarise(meanC = mean(Totals))
