...
test <- read.xlsx("FOI IMT178324 SuffolkCoastal.xlsx")

unique(test$Exemption.type)
...
colour = alpha("darkred", 1/2), size = 0.1) + scale_colour_gradient2() +
  labs(title = "9 Boroughs: Ward Level % Empty") +
  theme(plot.title = element_text(hjust = 0.5))+
...
CHELDAT %>%
  mutate(LowUsePerc = LowUse/WardHomes*100, 
         ...
         EmptyPercRank = percent_rank(Empty/WardHomes)) %>%
ggplot(.)+ geom_polygon( aes(long, lat, group = group, fill = LowUsePerc), 
colour = alpha("darkred", 1/2), size = 0.2) + scale_colour_gradient2() +
  labs(title = "Kensington and Chealsea: Ward Level % Low Use") +
  theme(plot.title = element_text(hjust = 0.5))+ 
...
AG %>% filter(grepl("Kensington", LAD11NM)) %>% 
  left_join(., select(Chelsea, LSOA_CODE, LowUse, Empty, Homes), by = c("lsoa11cd"="LSOA_CODE")) %>% 
  ungroup %>%
  filter(WD16NM!= "Kensal Green") %>%
  mutate(LowUsePerc = LowUse/Homes.y*100, 
         EmptyPerc = Empty/Homes.y*100,
...
Cornwall %>% mutate(LowUsePerc = LowUsePerc*100) %>% 
  left_join(., AG, by = c("LSOA_CODE" = "lsoa11cd"))  %>% 
  ggplot(.)+ 
  geom_polygon( aes(long, lat, group = group, fill = LowUsePerc), 
colour = alpha("darkred", 1/2), size = 0.2) + scale_colour_gradient2() + ggtitle("Cornwall: Ward level Empty and Second Homes %")+
  theme(plot.title = element_text(hjust = 0.5)) + 
  scale_fill_continuous(name = "Percentage")
...
CornCor <- Cornwall %>% 
 ...
 #"Cornwall: Relationship between LowUse\n Percentage and Mean Price in Ward"
