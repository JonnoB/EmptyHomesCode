...
test3 <- Chelseainfo %>% 
  ...
  left_join(., test, "Admin_ward_code") %>%
  ...
  mutate(PercOfLowUse = LowUseClass/sum(LowUseClass),
         ...
         `Actual Homes` = PercOfLowUse*sum(LowUseClass))
...
test <- unique(ChelPrice$lsoa11cd)%>% 
  map(~ ChelPrice %>% 
        filter(lsoa11cd == .x) %>% 
  ClassStrapper(., sum((filter(Chelseainfo, LSOA_CODE == .x))$LowUse)) %>%
