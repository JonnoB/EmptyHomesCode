...
#Haringey is funney and has to be worked over a bit first
...
PlotMap(Lambethinfo, AG)
 ...
 ls(pattern = "info") %>% map_df(~eval(parse(text=.x))) %>%
  PlotMap(., filter(AG, RGN15NM=="London"), filtermap = FALSE)
SaveFig("LondonCont.png")
 
 ls(pattern = "info") %>% map_df(~eval(parse(text=.x))) %>%
  PlotMap(., filter(AG, RGN15NM=="London"), variable = "LowuseClass", filtermap = FALSE)
...
PlotMap(Chelseainfo, AG, title = "Low-use homes in Kensington and Chelsea")
...
ChelseaCor <- Chelseainfo %>% 
  summarise(x =cor(MedianPrice, WardLowUsePerc, use = 'complete.obs')) 

 Chelseainfo %>% ggplot(., aes(y=MedianPrice, x= WardLowUsePerc )) + geom_point() +
   labs(title = "Median Price vs Low Use % by ward in RBKC",
        x = "Low Use Percentage by Ward", 
        y = "Median Price") +
  theme(plot.title = element_text(hjust = 0.5)) + 
    annotate("text", 
             label = paste0("Correlation = ", 
             signif(ChelseaCor[1,1],2)), 
             x = 5 , y = 3.5e5, size = 5) +
...
PlotMap(test, AG, variable = "WardLowUsePerc")
