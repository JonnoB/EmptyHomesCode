...
if(file.exists("BootStrapRES.rds")){
 #this sectio of code loads the old list and appends the new LADs
  
  BootStrapRES<- readRDS("BootStrapRES.rds")
  newLADsnames <- sub("DATA", "", ls(pattern = "DATA")) %>% 
 ...
 #re-roder the whole lot to be alphabetical
...
test <- BootStrapRES$CoastWave
#look at specific data
BootStrapRES$SouthwarkLON %>% mutate(RatioExvsAct = (RatioExvsAct-1)*100)%>% 
  ggplot(., aes(x= class, y = RatioExvsAct, fill = class)) + geom_boxplot() + 
  labs(title = "Bootstrapped difference from expected number of low-use homes", y = "% difference from expected")
