...
#Calculate Quantiles
 ...
 This is not the right way to do it but give almost identical values to the weighted sampled quantiles. 
 if you don't believe me see "MSOAmodel.Rmd" section "Calc Real Quantiles for all data". It takes ages and you regret everything

```{r}

 PropertyTypes <- c("D", "S", "T", "F")   #I need to specify this is what is being included in write up

  #Adds in Quantiles based on the LADs we have data for
  prices <- prices %>%
    mutate(CountryClass = cut(X2, 
                              X2 %>% .[LAD11CD %in% unique(DATAdf$LAD11CD)] %>% quantile(.) %>%
                                .[2:4] %>% c(0,., Inf), 
...
BootstrapAllData(DATAdf, LimitValue = 2e5, Reps = 501, GroupingVars = "MSOA11CD")
...
test1 <- system.time(result <-  StrappedLowMem(dfPrice, dfWard, samples, Grouping = GroupVars))
test2 <- system.time(result <-  StrappedHighMem(dfPrice, dfWard, samples, Grouping = GroupVars))
...
test <- BootStrapQuartiles$E09000020

test <-DistribCompareBootstrapper(DATAdf%>%filter(LAD11CD == "E09000020"), samples = 100)
...
test3 <-test%>%
  group_by(ID)%>%
  mutate(RatioExvsAct = Homes/sum(Homes)) %>%
  mutate(RatioExvsAct = (RatioExvsAct/0.25)-1) %>%
    ungroup 

test2 %>% PlotClassDiff()

CleanForPlotClassDiff(BootStrapQuartiles$E09000020)  %>%
