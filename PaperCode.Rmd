...
BradfordDATA %>% filter(LAD11NM=="Bradford") %>%
  MakeLeaflet
...
cor(unafford$Yearly.income, unafford$MedianPrice, "complete.obs")
...
#This is no Longer useful due to using the whole percentile range 
# test <- 2:50 %>% 
#   map_df(~{
#   decile <- unaffordExLon %>%
#     mutate(decilev = ntile(ratio, .x)) %>%
#     filter(decilev == 1 | decilev == .x) %>%
#     group_by(decilev) %>%
#     summarise(mean = mean(LowUsePerc)) %>%
#     mutate(group = .x) %>%
#     spread(key = decilev, value = mean) %>%
#     setNames(c("group", "LowMean", "HighMean"))
#   
# }) 
# 
# test %>%
#   gather(key = type, value = mean, -group)%>%  
#   ggplot(aes(x = type, y = mean )) + geom_boxplot()
# 
# 
# test %>%
#   gather(key = type, value = mean, -group)%>% 
#   ggplot(aes(x = group, y = mean, colour = type )) + geom_point()
# 
# 
# test2 <- test %>%
#   mutate(ratio = HighMean/LowMean) 
# test2 %>%
#   ggplot(aes(x = group, y = ratio,)) + geom_point()
# 
# cor(test2$ratio, test2$group)


decile <- bind_rows(mutate(unaffordExLon, type = "Excl London"), 
                    mutate(unafford, type = "Inc London"),
                    mutate(unaffordLon, type = "London")) %>%
    group_by(type) %>%
    mutate(decile = ntile(ratio, 50)) %>%
...
decile %>% ggplot() + 
  ...
  geom_point(aes(x= decile, y = median, colour = type))  
...
MultiCheck <- 1:111 %>% map(~{
  ...
  model <- rpart(Reference~ Extent + price + VacantsPerc+ PercRural,data=as.data.frame(TestResample))
...
BinomExtent <- glm(Reference~ Extent + price + VacantsPerc, family=binomial(link='logit'),data=ModelMe)
rpartExtent <- rpart(Reference~ Extent + price + VacantsPerc,data=ModelMe)

confusionMatrix((predict(BinomExtent, ModelMe, type = "response")>0.5) , ModelMe$Reference)
confusionMatrix(predict(rpartExtent, ModelMe, type = "prob")[,2] , ModelMe$Reference)
