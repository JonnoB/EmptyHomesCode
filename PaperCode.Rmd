...
#Conway have sen the wrong stuff  
# ConwayDATA <- read_excel("Conwy FOI 0260-17.xlsx" , sheet=1) %>%
...
```{r}
test <- BootStrapRESExO$ChelseaLON %>% 
  group_by(class) %>%
...
BootStrapRES$CoLLON %>% PlotClassDiff()
BootStrapRESExO$CoLLON %>% PlotClassDiff()
BootStrapRESExOA$CoLLON %>% PlotClassDiff()
BootStrapRESExOB$CoLLON %>% PlotClassDiff()
...
#Look Kings Lynn and harrogate as they are strange results

KingsLynnDep <- KingsLynnDATA %>% left_join(LSOADep, by = "LSOA_CODE") %>% 
  mutate(deprank = rank(Decile..where.1.is.most.deprived.10..of.LSOAs.))
  
HarrogateDep <- HarrogateDATA %>% left_join(LSOADep, by = "LSOA_CODE") %>%
  mutate(deprank = rank(Decile..where.1.is.most.deprived.10..of.LSOAs.))
...
test %>% ggplot(.,aes(x= maxPerc, y =Ttest.p.value, colour = outcome)) +
  geom_jitter()


svm.model <- svm(as.factor(absdiff>0) ~ LUPdepExt+ maxPerc, test)
#why do the two rforest models have such different results
#rpart.model <- randomForest(as.factor((absdiff>0))~ LUPdepExt+ maxPerc, test)
rpart.model <- rpart(as.factor(absdiff>0) ~ LUPdepExt+ maxPerc, test)

confusionMatrix(predict(svm.model, test, type = class),as.factor(test$absdiff>0))
...
Combine1ExO[Combine1ExO$Ttest.p.value>0.5 & Combine1ExO$maxPerc>5,]
Combine2ExO[Combine2ExO$Ttest.p.value>0.5 & Combine2ExO$maxPerc>5,]
Combine3ExO[Combine3ExO$Ttest.p.value>0.5 & Combine3ExO$maxPerc>5,]
...
Combine1ExO %>% ggplot(.,aes(x= maxPerc, y = Ttest.p.value)) + geom_point() +
