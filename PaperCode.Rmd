...
t.test(ModelStats2$Accuracy, ModelStats1$Accuracy)
...
BinomSimp <- glm(Reference~ price + VacantsPerc, family=binomial(link='logit'),data=ModelMe)
...
confusionMatrix(predict(BinomSimp, ModelMe, type = "response")>0.5 , ModelMe$Reference)
...
EnglandComp <- LADDep %>% 
  ...
  mutate(VacantsPerc = Vacants/Homes) %>%
  mutate(Significant.Competition = predict(BinomExtent, ., type = "response")) %>%
