...
# WiltshireDATA <- read_excel("Wiltshire ENQ07536 - Council tax discounts.xlsx",
#                                sheet=1)[1:4] %>%
#   StructureData()
```
...
CurrentLADS <- ls(pattern = "DATA") %>%
  map_chr(~{df<- get(.x)
  df$LAD11CD[which.max(table(df$LAD11CD) %>% as.matrix)] }
)
...
HouseMatch <- data_frame(PropType = "D,S,T,F,O" %>% str_split(",") %>% unlist,
type = "Detached,Semi-Detached,Terraced,Flats/Maisonettes,Other"%>% str_split(",") %>% unlist)

#test CoL after adding in filtering for other LADs
testCol <- DistribCompareBootstrapper(CoLLONDATA,1652, 1000)
testColA <- DistribCompareBootstrapper(CoLLONDATA,1652, 1000, "A")
testColB <- DistribCompareBootstrapper(CoLLONDATA,1652, 1000, "B")

testColA %>% mutate(RatioExvsAct = (RatioExvsAct-1)*100)%>% 
  ggplot(., aes(x= class, y = RatioExvsAct, fill = class)) + geom_boxplot() + 
  labs(title = "Bootstrapped difference from expected number of low-use homes", y = "% difference from expected")
...
test <- CoLLONDATA %>% SubsetPrice() %>% left_join(HouseMatch)
testB <- CoLLONDATA %>% SubsetPrice("B") %>% left_join(HouseMatch)
testA <- CoLLONDATA %>% SubsetPrice("A") %>% left_join(HouseMatch)

testB <- ChelseaLONDATA %>% SubsetPrice("B") %>% left_join(HouseMatch)
testA <- ChelseaLONDATA %>% SubsetPrice("A") %>% left_join(HouseMatch)

testB <- ChelseaLONDATA %>% SubsetPrice("B") %>% left_join(HouseMatch)
testA <- ChelseaLONDATA %>% SubsetPrice("A") %>% left_join(HouseMatch)

test <- CoLLONDATA %>% SubsetPrice("A")

CoLnums <- BootStrapRES$CoLLON %>% 
  group_by(class) %>%
  summarise_all(funs(mean(., na.rm = TRUE)))


CoLnumsA <- BootStrapRESA$CoLLON %>% 
  group_by(class) %>%
  summarise_all(funs(mean(., na.rm = TRUE)))

table(test$type, test$class)
table(testA$type, testA$class)
table(testB$type, testB$class)
...
sum(prices$X15 == "B")/nrow(prices)

test <- ABratio4 %>% 
  filter(LAD11CD %in% CurrentLADS)

ABratio3 %>% ggplot(., aes(x = Valueratio)) +geom_density()
...
test %>%
  ggplot(.,aes(x= diffPerc, y = Ttest.p.value, colour = Countratio>0.2)) + geom_point()

test %>%
  ggplot(.,aes(x= diffPerc, y = Ttest.p.value, colour = mean)) + geom_point()
