...
install.packages("breakDown") provides a plot of the glm!
...
DataCover <- DATAdf %>% 
  ...
  summarise(Pop = sum(Pop),
            Homes = sum(Homes),
            LUPs = sum(LowUse),
            LSOA = length(unique(LSOA11CD)),
            MSOA = length(unique(MSOA11CD)),
            Region = first(Region),
            LAD = 1) %>%
  select(-LAD11CD) %>%
  group_by(Region) %>%
  ...
  left_join(.,
            MSOAtoLAD %>%
  select(LAD11CD, Region, LAD11NM) %>%
  distinct(LAD11CD, .keep_all = T) %>%
  group_by(Region) %>%
  summarise(LADPerc = n())
) %>% 
  mutate(LADPerc = LAD/LADPerc,
         Region2 = c("North East", "North West", "York and Humber", "West Midlands", "East England", "London", "South East", "South West", "Wales")) %>% 
  arrange(Region)
  
Total <- DataCover %>% summarise_if(is.numeric, sum) %>%
  mutate(
    Region = "Total",
    LADPerc = LAD/348)

DataCover <-bind_rows(DataCover, Total) %>%
  mutate(LADPerc = round(LADPerc*100)) %>%
  rename(`% of LADs` = LADPerc) %>%
  mutate_if(is.numeric, as.integer)

DataCover 
DataCover %>% mutate(Perc = LUPs/Homes)

xtable(DataCover, caption="Summary of collected dataset",
       label = "tab:DataCover") %>% 
  print(., type="latex", file=file.path(TexTables,"DataCover.tex"), include.rownames=FALSE)

rm(DataCover);rm(Total)
...
##Non-Dataset LADs
This bootstrapping is done to get the data needed to predict across the whole of England and Wales
```{r}
...
CleanForPlotClassDiff(All_data_class, CountryClass) %>% mutate(Area = "All Data")
  ...
  PlotClassDiff(Var = "LowUseRatio1") +
    facet_wrap(.~Area) +
  ggtitle("Low-Use Property distribution by affordability Quartile")
...
#Compare the Long term vacants data with the collected data

    test <-file.path(DataFolder, "LT_615.xls") %>%
      read_xls(., sheet = 2, skip = 5) %>%
      set_names(make.names(names(.))) %>%
      select(LAD11CD = New.ONS.code, X2017) %>%
      left_join(select(LADMean, LAD11CD, LowUse),.) %>%
      filter(complete.cases(.))
    
    cor(test$LowUse, test$X2017)
    
    
    test <-file.path(DataFolder, "LT_615.xls") %>%
      read_xls(., sheet = 3, skip = 5) %>%
      set_names(make.names(names(.))) %>%
      select(LAD11CD = New.ONS.code, X2017) %>%
      left_join(select(LADMean, LAD11CD, LowUse),.) %>%
      filter(complete.cases(.))
...
#2nd homes and Tourism
```{r}

TourismLAD <- Tourism %>%
  group_by(LAD11CD) %>%
  summarise_if(is.numeric, sum, na.rm = T) %>%
  bind_cols(Tourism %>% group_by(LAD11CD) %>% summarise(LAD11NM =first(LAD11NM)),.) %>%
  ungroup

ScndhomesCensus <- read_xlsx(path = file.path(DataFolder, "Census2011secondHomes.xlsx"), sheet = 3, skip = 13)[,1:7] %>%
  mutate(LAD11NM = ifelse(is.na(X__1), X__2, X__1)) %>%
  select(LAD11CD = K04000001, LAD11NM, ScndHomes = X__5) %>% # Pop = X__4,
  filter(!is.na(ScndHomes)) %>%
  #Replace changed codes
  mutate(LAD11CD = case_when(LAD11NM == "St Albans"~"E07000240",
                             LAD11NM =="Welwyn Hatfield"~"E07000241",
                             LAD11NM =="Northumberland UA 6"~"E06000057",
                             LAD11NM =="East Hertfordshire"~"E07000242",
                             LAD11NM =="Stevenage"~"E07000243",
                             LAD11NM =="Gateshead"~"E08000037",
                             TRUE~ LAD11CD)) %>%
  select(-LAD11NM) %>%
  left_join(TourismLAD, ., by = "LAD11CD") %>%
  mutate(TourismDens = (Guest + Hotel)/Homes,
         ScndHomesDens = ScndHomes/Homes) %>%
  filter(!is.na(LAD11CD)) %>%
  mutate(Perc2ndHomes = ScndHomes/sum(ScndHomes))

#not a lot of corellation
cor(ScndhomesCensus$TourismDens, ScndhomesCensus$ScndHomesDens)

#no real pattern
ScndhomesCensus %>%
  filter(TourismDens<0.1) %>%
ggplot(aes(x = TourismDens, ScndHomesDens)) + geom_point()

test <- left_join(LADMean, ScndhomesCensus, by = "LAD11CD")

sum(test$Perc2ndHomes)
...
bind_rows(MSOAMean %>% mutate(type = "MSOA"), LADMean %>% mutate(type = "LAD"))  %>%group_by(type)%>% mutate(Eg2 = percent_rank(Ego2TourismDens)) %>% ungroup %>%
  ...
  facet_wrap(~type) + 
  labs(title = "Does the geography have above the median percent of LUPs", x = "Affordability Rank", y = "log Tourism Rank") +
  guides(colour=guide_legend(title="Above Median"))+
...
bind_rows(MSOAMean %>% mutate(type = "MSOA"), LADMean %>% mutate(type = "LAD")) %>%
  ...
  facet_wrap(~type) + 
  labs(title = "Are LUPs more expensive than homes?", x = "Affordability Rank", y = "log Tourism Density") +
  guides(colour=guide_legend(title="More expensive"))+
  theme(  legend.position="bottom")
SaveFig("HighVal.pdf")
...
WelshTax <- read_csv("WelshCTax.csv")

WelshTax <- read_csv("WelshCTax.csv", skip = 4, n_max = 23) %>%
  select(-X1) %>%
...
test <- EmptyTax %>%
  select(LAD11CD, LAD11NM, Homes, LowUse, HomesMedian, LowUseMedian, LowUsePerc, MedianDiff, MedianDiffRatio, EmptyTax, EmptyOfCouncil, TaxPop)
write_csv(test, file.path(basewd, "Key Figures by Council.csv"))
...
sum(EmptyTax$EmptyTax*100)/sum(EmptyTax$LowUse)

sum(EmptyTax$AllPropVal)/sum(EmptyTax$Homes)
...
MSOAAffordRatio  <- bind_rows(MSOAAffordRatio %>% filter(Region != "E12000007") %>% mutate(type = "Excluding London"), 
                    MSOAAffordRatio %>% mutate( type = "Including London"),
                    MSOAAffordRatio %>% filter(Region == "E12000007") %>% mutate( type = "Only London")) %>%
    group_by(type) %>%
    mutate(
...
data_frame(Index = 1:100) %>%
  mutate(`Low Value` = exp(-0.05*(Index))+0.05, 
         `High Value` = (exp(Index/100)-1),
    Total = `Low Value` + `High Value`) %>%
  gather(key = "Type", value = "value", -Index) %>%
  ggplot(aes(x= Index, y = value, colour = Type )) +
  geom_line() +
  labs(title = "LUP percentage VS affordability ratio",
...
LADModTab <-LADModels %>% SummariseModels(Dependent, IncAfford2,  IncTourism) %>%
  mutate(Dependent = case_when(
    Dependent=="HighVal"~ "High-Value",
    Dependent == "HighLUP"~ "High-LUP %",
    TRUE ~ "HVHL"
  ) %>% as.factor) %>%
  mutate(ModelID = rep(1:4,3)) %>%
  arrange(Dependent)

LADModTab %>% select(ID = ModelID, Dependent, `Beats Null`=`Beats NULL`, Accuracy=`Mean Accuracy`, PosPred, NegPred, Lift) %>%
  slice(c(2,8,10)) %>% 
  mutate(ID = ifelse(ID == 2, 3, ID))%>%
  xtable(., caption="Summary of viable Local Authority models",
       label = "tab:LADmodels") %>% 
  print(., type="latex", file=file.path(TexTables,"LADmodels.tex"), include.rownames=FALSE)
...
MSOAModTab <- MSOAModels %>% SummariseModels(Dependent, IncAfford2,  IncTourism) %>%
  mutate(Dependent = ifelse(Dependent=="HighLUP", "High-LUP %", "High-Value")) %>%
  mutate(ModelID = rep(1:4,2))

MSOAModTab %>% select(ID = ModelID, Dependent, `Beats Null`=`Beats NULL`, Accuracy=`Mean Accuracy`, PosPred, NegPred, Lift) %>%
  slice(c(4)) %>%
  xtable(., caption="Summary of viable MSOA authority models",
       label = "tab:MSOAmodels") %>% 
  print(., type="latex", file=file.path(TexTables,"MSOAmodels.tex"), include.rownames=FALSE)
...
bind_rows(LADMODOut %>% mutate(type = "High-Value (LAD Level)"),
          LADMODOut3 %>% mutate(type = "HVHL (LAD Level)"),
          LADMODOut2 %>% mutate(type = "High-LUP % (LAD Level)"),
          MSOAMODOut %>% mutate(type = "High-LUP % (MSOA Level)")
) %>%
...
FormulaBootLAD<-c(as.formula(Reference ~ TourismDensity + AffordRatio),
  as.formula(Reference ~ TourismDensity + AffordRatio + AffordRatioScale2),
                  as.formula(Reference ~ TourismDensity + AffordRatio))
...
EWMod <- LADMean %>% 
  rename_(Reference = c("HighVal", "HighLUP", "HighFinance")[.x] )  %>% 
  mutate(Reference = as.factor(Reference)) %>%
      glm(formula = FormulaBootLAD[[.x]], data = ., family=binomial(link='logit'))

    
    PredictData  %>%
      predict(EWMod, newdata = ., type = "response" ) %>%
      as.tibble(.) %>% setNames(c("HighVal", "HighLUP", "HighFinance")[.x])
...
PopAffected %>% 
  ...
  gather(key, value) %>%
  mutate(key = ifelse(key == "PopPerc", "High-Value", "High-Value High-LUP%")) %>%
...
LADShape2 <- WholeCountry %>%
  ...
  mutate(Type = case_when(
    Type =="HighFinance" ~ "High-Value High-LUP",
    Type == "HighLUP" ~"High LUP percentage",
    Type == "HighVal" ~"High-Value LUPs"
  ) %>% as.factor(.) %>% fct_relevel("High-Value High-LUP", after = 2),
...
ggplot(LADShape2) +
  ...
  facet_wrap(~Type)+
  labs(title = "Predicting LUP status across England and Wales") +
...
#Gets the info on how conentrated the empty homes are
test <- DATAdf %>%
  arrange(-LowUsePerc) %>%
  filter(!is.na(MSOA11CD)) %>%
  mutate(CumsumLowUse = cumsum(LowUse),
         CumsumHomes = cumsum(Homes),
         CumsumLowUsePerc = CumsumLowUse/sum(LowUse),
         CumsumHomesPerc = CumsumHomes/sum(Homes),
         CumPerc = 1:nrow(.)/nrow(.) )

median(test$LowUsePerc)
mean(test$LowUsePerc)
max(test$LowUsePerc)

test[(c(0.5,2,5)/10) %>% map_dbl(~Position(function(fi) fi <= .x, test$CumPerc, right = TRUE)),] %>%
  select(CumsumLowUsePerc, CumsumHomesPerc, CumPerc)

#Dumps a CSV of all the LADs is not necessary for script and so is commented out, The csv can then be used to add the file names and FOI references
# 
# DATAdf %>%
#   select(LAD11CD, LAD11NM, RGNCD = Region) %>%
#   distinct(LAD11CD, .keep_all = TRUE) %>%
#   left_join(data_frame(RGNCD = c("E12000001", "E12000002", "E12000003", 
# "E12000005", "E12000006", "E12000007", "E12000008", "E12000009", 
# "W99999999"), Region = c("North East", "North West", "York and Humber", 
# "West Midlands", "East England", "London", "South East", "South West", 
# "Wales"))) %>%
#   write_csv(file.path(basewd,"LADsDataRaw.csv"))
...
#The top 3 LSOA for each LAD in terms of perc low use
TopLSOA <- DATAdf %>% 
  ungroup %>%
  arrange(-LowUsePerc) %>%
  group_by(LAD11CD) %>%
...
#The value of the low use property
TotalValueProperty <- DATAdf %>%
  group_by(LAD11CD) %>%
  summarise(LAD11NM = first(LAD11NM),
            LUPSValue = sum(LowUse*MeanPrice, na.rm = T),
            HomesValue = sum(Homes*MeanPrice, na.rm = T)) %>%
  mutate(LUPofTotMarket = LUPSValue/HomesValue)

Key_Areas <- read.csv(file.path("/home/jonno/Downloads", "LAs for Jonathan.csv")) %>% select(LAD11CD, news_area) 

TopLSOA <- Key_Areas %>%
  left_join(TopLSOA, .) %>%
  filter(!is.na(news_area))

TotalValueProperty <- Key_Areas %>%
  left_join(.,TotalValueProperty)

test <-LADMean %>%
 #filter(LAD11NM == "Barnet")
  filter(Region == "E12000007") %>%
  select(LAD11NM, LowUse, LowUseMean, HomesMean, AffordRatio, MedianDiff, MedianDiffRatio) %>%
  mutate_if(is.numeric, signif, digits = 3)

write.csv(test, file.path("/home/jonno", "LondonMSOA.csv"))
  
test <- DATAdf %>%
filter(LAD11CD == "E06000052") %>%
  write.csv(file.path("/home/jonno/Downloads", "Brighton.csv"))

write.csv(TopLSOA, file.path("/home/jonno/Downloads", "TopLSOA.csv"))
write.csv(TotalValueProperty, file.path("/home/jonno/Downloads", "TotalValueProperty.csv"))


LSOAshape <- st_read(LSOAshapedata) %>%
  left_join(DATAdf, by = c("lsoa11cd"="LSOA11CD")) %>% filter(grepl("Bradford",LAD11NM)) #filter(grepl("E09000003",LAD11CD))
...
LSOAshape %>%
ggplot(.)+  geom_sf(aes(fill = LowUsePerc))  +
   theme(axis.title.x=element_blank(),
          axis.text.x=element_blank(),
          axis.ticks.x=element_blank(),
          axis.title.y=element_blank(),
          axis.text.y=element_blank(),
          axis.ticks.y=element_blank())+
  ggtitle("Low use Percentage by lsoa in Trafford")+
  scale_fill_viridis_c()


test <- TotalValueProperty  %>%
  filter(grepl("(Allerdale)|(Copeland)|(Carlisle)|(South Lake)|(Barrow)|(Eden)", LAD11NM))
...
CorePstCd %>%
 filter(Region == "E12000007") %>%
  distinct(LAD11CD, .keep_all = T) %>%
  select(LAD11CD, LAD11NM) %>%
  filter(!(LAD11CD %in% (LADMean %>%  filter(Region == "E12000007") %>% pull(LAD11CD))))
