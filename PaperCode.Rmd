...
BootStrapRESExO2 <- BootStrapRESExO %>% map(~.x %>% ungroup)

LADnames <- names(BootStrapRESExO)

LUPval <-   map2(.x =BootStrapRESExO2, .y =   names(BootStrapRESExO2),~ {.x %>% mutate(LAD  = .y)}) %>%
  bind_rows %>%
  select(-class) %>%
  group_by(LAD, ID) %>%
  summarise_all(sum) %>%
...
test <- ModelStats %>% 
  select( X.Intercept., LUPdepExt, price, VacantsPerc) %>%
  gather 
...
Outcome <- bind_rows(data_frame(model = "Open Data", Accuracy = ModelStats2$Accuracy), 
data_frame(model = "cutsom", Accuracy = ModelStats3$Accuracy) )

Outcome %>%
  ggplot(.,aes(x = Accuracy, fill = model)) + geom_density(alpha = 0.3) +
  labs(title = "Density plot of the model outcomes, using custom FOI data vs generic open data")
SaveFig("CustomGenericOutcomes.pdf")
...
ModelStats2 %>%
  select(X.Intercept. ,Extent,price , VacantsPerc) %>%
  gather() %>%
  ggplot(aes(x= key, y = value, fill = key)) +geom_boxplot() +
  labs(title = "Coefficient values across 1000 random partitions")
SaveFig("ModelCoeffs.pdf")


#only one of these is necessary they hold almost the same information
ModelStats3 %>%
  select(X.Intercept. ,LUPdepExt,price , VacantsPerc) %>%
  gather() %>%
  ggplot(aes(x= key, y = value, fill = key)) +geom_boxplot() +
...
EnglandCompTab <- EnglandComp %>%
  ...
  summarise(mean = mean(price),
            ...
            Homes = sum(Homes),
            Pop = sum(Pop)) %>%
  mutate(HomesPerc = Homes/sum(Homes)*100,
         PopPerc = Pop/sum(Pop)*100) %>%
  rename('LUPs>Homes' = Comp.Type, 
         Population = Pop,
         'Homes %' = HomesPerc,
         'Population %' = PopPerc) %>%
  gather(key = "key", "value"= value, -"LUPs>Homes") %>%
  mutate(value = value %>% round %>%as.integer) %>%
  spread(key = "LUPs>Homes", value = value)

EnglandCompTab 

xtable(EnglandCompTab, caption="Summary of effect across England",
...
#Plotting the expected range of 

valuecheck <- sample(c(TRUE, FALSE), size = nrow(EnglandComp)*1000, replace = TRUE, prob = c(0.888, 1-0.888)) %>%
  matrix(., nrow=nrow(EnglandComp)) 
valuecheck <-   (valuecheck * ifelse(EnglandComp$Comp.Type == "Yes", TRUE, FALSE ) )%>% data.frame %>%
  bind_cols(select(EnglandComp, LAD11CD, Reference),.) %>%
  #Where ever the outcome is known it is reverted back to the true value, only estimated values are flipped.
  gather(key = "ID", value = "affected",-LAD11CD,-Reference) %>%
  mutate(Reference = as.logical(Reference)*1,
    affected = ifelse(is.na(Reference), affected, Reference)) %>%
  left_join(LadHomes) %>%
  group_by(ID, affected) %>%
  summarise(Homes = sum(Homes),
            Pop = sum(Pop)) %>%
  group_by(affected) %>%
  mutate(Rank = percent_rank(Pop),
    Outlier = ifelse(percent_rank(Pop)<0.0275 | percent_rank(Pop)>0.975, "Yes", "No")) %>%
  group_by(ID) %>%
  mutate(affected = ifelse(affected == 1, "Yes", "No"),
         PercPop = Pop/sum(Pop))%>%
  ungroup #%>%
# mutate(PercPop = Pop)
#The NNUMBERS DONT MATCH WITH THE TABLE WHY?


#95 range is 78 to 84
affected <- valuecheck %>% filter(affected == "Yes")
q5 <- quantile(affected$PercPop,.05)
q95 <- quantile(affected$PercPop,.95)
x.dens <- density(affected$PercPop)
df.dens <- data.frame(x = x.dens$x, y = x.dens$y)
median(affected$PercPop)
affected %>%
  ggplot(aes(x = PercPop)) + geom_density() +
  geom_area(data = subset(df.dens, x <= q5), 
              aes(x=x,y=y))  +
    geom_area(data = subset(df.dens, x >= q95), 
              aes(x=x,y=y))+
labs(title = "The estimated percentage of English population affected by LUPs")
SaveFig("PopPercLUPAfftect.pdf")



valuecheck %>% filter(affected == "No") %>%
  ggplot(aes(x = PercPop)) + geom_density()
